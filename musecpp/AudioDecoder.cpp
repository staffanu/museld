    ...
    while (m_queue.size() >= 1350) {
        ...
        if (m_consecutive_failed_syncs <= 2 && m_queue.size() >= 1350 - 16) {
            ...
            if (m_active_audio_mode == MODE_A) {
                ...
                for (int i = 0; i < 16; i++) {
                    output_samples[0][sample_count] = aModeChannel1Decoder.decodeSample(range1, range1ok, &deinterleave_matrix[i][1], row_ok[i]);
                    output_samples[1][sample_count] = aModeChannel2Decoder.decodeSample(range2, range2ok, &deinterleave_matrix[i][17], row_ok[i]);
                    output_samples[2][sample_count] = aModeChannel3Decoder.decodeSample(range3, range3ok, &deinterleave_matrix[i][34], row_ok[i]);
                    output_samples[3][sample_count] = aModeChannel4Decoder.decodeSample(range4, range4ok, &deinterleave_matrix[i][50], row_ok[i]);
                    sample_count++;
                }
                for (int i = 0; i < 16; i++) {
                    output_samples[0][sample_count] = aModeChannel1Decoder.decodeSample(range1, range1ok, &deinterleave_matrix[i][9], row_ok[i]);
                    output_samples[1][sample_count] = aModeChannel2Decoder.decodeSample(range2, range2ok, &deinterleave_matrix[i][25], row_ok[i]);
                    output_samples[2][sample_count] = aModeChannel3Decoder.decodeSample(range3, range3ok, &deinterleave_matrix[i][42], row_ok[i]);
                    output_samples[3][sample_count] = aModeChannel4Decoder.decodeSample(range4, range4ok, &deinterleave_matrix[i][58], row_ok[i]);
                    sample_count++;
                }
                ...
                for (int i = 0; i < 16; i++) {
                    output_samples[0][sample_count] = bModeChannel1Decoder.decodeSample(range1, range1ok, &deinterleave_matrix[i][12], row_ok[i]);
                    output_samples[1][sample_count] = bModeChannel2Decoder.decodeSample(range2, range2ok, &deinterleave_matrix[i][23], row_ok[i]);
                    sample_count++;
                }
                for (int i = 0; i < 16; i++) {
                    output_samples[0][sample_count] = bModeChannel1Decoder.decodeSample(range1, range1ok, &deinterleave_matrix[i][45], row_ok[i]);
                    output_samples[1][sample_count] = bModeChannel2Decoder.decodeSample(range2, range2ok, &deinterleave_matrix[i][56], row_ok[i]);
                    sample_count++;
                }
            } else
