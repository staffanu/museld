...
void print_max_and_avg(string header, uint8_t const *data, size_t length) {
    uint8_t m = 0;
    long sum = 0;
...
void VideoDecoder::DecodeSingleField(FieldBufferView &field, Mat &field_out) {
    ...
    print_max_and_avg("interpolated32", interpolated32[0], MUSE_BUF_HEIGHT * MUSE_Y_BUF_WIDTH * 2);
    ...
    // print_max_and_avg("48 MHz", field_out.data, MUSE_BUF_HEIGHT * 2 * MUSE_Y_BUF_WIDTH * 3);
    ...
    for (int row = 0; row < MUSE_BUF_HEIGHT; row++) {
        ...
        for (int col = 0; col < MUSE_Y_BUF_WIDTH * 3; col++) {
            ...
            int row_below = min(empty_row + 1, MUSE_BUF_HEIGHT * 2 - 1);
    ...
    // print_max_and_avg("Output", field_out.data, MUSE_BUF_HEIGHT * 2 * MUSE_Y_BUF_WIDTH * 3);
}
