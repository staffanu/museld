...
namespace musevk {
    ...
    public:
        ...
        static std::unique_ptr<VulkanBuffer> createDeviceBufferFloatsAsHalfFloats(VulkanManager &vulkan_manager, Size const &size, const std::vector<float> &data) {
            assert(size.numberOfElements() == data.size());
            std::vector<ushort> half_floats(data.size());
            for (int i = 0; i < data.size(); i++)
                half_floats[i] = HalfFloatUtil::float_to_half(data[i]);
            return createDeviceBuffer<ushort>(vulkan_manager, size, half_floats);
        }
    };
}
