...
bool ResamplingFrameReader::resample(float *sample_out, uint8_t *dropout_out,
    ...
    const float b0 = m_input_buffer[i3];
    const float b1 = m_input_buffer[i2];
    const float b2 = m_input_buffer[i1];
    const float b3 = m_input_buffer[i0];

    *sample_out = cubicInterpolate(b0, b1, b2, b3, (float)(m_t - (size_t)m_t));
