...
ArErasureConcealer::ArErasureConcealer(Logger &log, int order, int hop_size, std::optional<std::string> debug_filename) :
...
void ArErasureConcealer::processFrame(std::span<TwoChannelSampleWithErasureFlags> samples, std::vector<double> &left_out, std::vector<double> &right_out) const {
...
bool ArErasureConcealer::isPresent(int value, const std::vector<int> &x) {
...
void ArErasureConcealer::reconstruct_signal(std::vector<double> const &samples, std::vector<bool> const &burst, std::vector<double> &output) const {
...
void ArErasureConcealer::update_a(const std::vector<double> &y, std::vector<double> &a) const {
    double R[m_order+1];
...
bool ArErasureConcealer::update_x(const std::vector<double> &y, const std::vector<double> &a, const std::vector<int> &t,
...
bool ArErasureConcealer::solveCholesky(const std::vector<std::vector<double> > &A, std::vector<double> &x) {
    const int N = x.size();
    assert(A.size() == N);
